...
public class Compat_Recipes_MineFactoryReloaded extends CompatMods {
	...
	@Override public void onPostLoad(FMLPostInitializationEvent aInitEvent) {
		...
		CR.shaped(ST.make(MD.MFR, "machine.2" , 1, 0), CR.DEF_REV_NCC, new Object[] {"CDC", "DMD", "CDC", 'C', OD_CIRCUITS[6], 'D', OP.lens.dat(ANY.Diamond), 'M', OP.casingMachineDense.dat(MT.Ad)});
		CR.shaped(ST.make(MD.MFR, "machine.2" , 1, 1), CR.DEF_REV_NCC, new Object[] {"CSC", "LML", "CDC", 'C', OD_CIRCUITS[6], 'D', OP.lens.dat(ANY.Diamond), 'M', OP.casingMachineDense.dat(MT.Ad), 'L', IL.Comp_Laser_Gas_CO2, 'S', ST.make(MD.MFR, "pinkslime", 1, 1)});
